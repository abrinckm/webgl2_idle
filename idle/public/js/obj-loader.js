class WavefrontLoader {
    ...
    static parseVertexData(data) {
        ...
        let normals = lines.reduce((acc,line)=>{if (line.startsWith("vn ")) {acc.push(line);} return acc;}, []);
        normals = normals.map(v => {let parts = v.split(" "); return [parseFloat(parts[1]), parseFloat(parts[2]), parseFloat(parts[3])]}).flat();
